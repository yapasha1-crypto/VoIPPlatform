...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (API_CONFIG.MOCK_MODE) {
        ...
        if (redirectCount > 3 && (now - lastRedirectTime) < 5000) {
            console.error('[Admin] ⚠️ REDIRECT LOOP DETECTED - Clearing session');
        ...
        if (userRole !== 'admin') {
            console.warn('[Admin] User is not admin, redirecting to appropriate dashboard');
    ...
    if (headerContainer) {
        ...
        try {
            ...
            headerContainer.appendChild(createHeader('admin', balanceData.balance || 0));
        } catch (error) {
...
async function loadDashboardStats() {
    try {
        ...
        // Total Users
        document.getElementById('stat-users').innerHTML = createStatCard({
        ...
        // Active Calls
        document.getElementById('stat-tickets').innerHTML = createStatCard({
        ...
        // Total SMS
        document.getElementById('stat-nodes').innerHTML = createStatCard({
