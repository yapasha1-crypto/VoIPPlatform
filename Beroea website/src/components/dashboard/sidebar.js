...
export function createSidebar(role = 'user') {
  const sidebar = document.createElement('aside');
  sidebar.className = 'sidebar scrollbar-custom';
  sidebar.id = 'dashboard-sidebar';

  const menuItems = {
    admin: [
      { icon: 'dashboard', label: 'Dashboard', href: 'admin.html', active: true },
    ...
    reseller: [
      { icon: 'dashboard', label: 'Dashboard', href: 'reseller.html', active: true },
    ...
    user: [
      { icon: 'dashboard', label: 'Dashboard', href: 'user.html', active: true },
    ...
    <div class="mt-auto pt-6 border-t border-white/10">
      <div class="flex items-center space-x-3 p-3 glass-card rounded-lg">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-white truncate">${role.charAt(0).toUpperCase() + role.slice(1)} User</p>
          <p class="text-xs text-white/40 truncate">user@beroea.com</p>
        </div>
      </div>
    </div>
